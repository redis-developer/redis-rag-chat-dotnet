...
using sk_webapi.Tokenizers;

namespace sk_webapi;

public class AllMiniLml6V2EmbeddingGenerator : ITextEmbeddingGenerator
{
    ...
    private static SentenceVectorizerAttribute _vectorizer = new ();

    public int CountTokens(string text) => Tokenizer.Value.Tokenize(text).Count;
    
    private static InferenceSession LoadInferenceSession()
    {
        var file = "Redis.OM.Vectorizers.AllMiniLML6V2.Resources.model.onnx";
        using var stream = Assembly.GetAssembly(typeof(SentenceVectorizer))?.GetManifestResourceStream(file);
        if (stream is null)
        {
    ...
    public Task<Embedding> GenerateEmbeddingAsync(string text, CancellationToken cancellationToken = new CancellationToken())
    {
        var vector = Vector.Of(text);
        vector.Embedding = Vectorize(text);
        var embedding = new Embedding(vector.Floats!);
